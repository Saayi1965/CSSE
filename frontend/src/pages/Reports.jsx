...
const IMAGES = {
  dashboardBg: 'https://images.unsplash.com/photo-1611273426858-450d8e3c9fce?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80',
  wasteCollection: 'https://images.unsplash.com/photo-1571624436279-b272aff752b5?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
  recycling: 'https://images.unsplash.com/photo-1587332065603-5e0e4bd833c1?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
...
function StatCard({ title, value, icon, color, delay = 0 }) {
  ...
  return (
    ...
    >
      ...
      <h6 className="fw-semibold mb-2" style={{ color: theme.textMuted, fontSize: '0.9rem' }}>
        {title}
      </h6>
      <h4 className="fw-bold mb-0" style={{ color: theme.textDark, fontSize: '1.5rem' }}>
        {value}
      </h4>
    </div>
  );
...
function CircularProgress({ value, delay = 0 }) {
  ...
  return (
    ...
    >
      ...
      <div className="small fw-semibold mt-2" style={{ color: theme.textMuted }}>
        Recycling Rate
      </div>
    </div>
  );
...
export default function Reports() {
  ...
  const [range, setRange] = useState({ start: '2025-10-01', end: '2025-10-15' });
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/reports/summary?startDate=${range.start}&endDate=${range.end}`);
      setData(res.data);
    } catch (err) {
      console.error(err);
      setData({
  ...
  const exportCsv = async () => {
    try {
      ...
      console.warn('Backend export failed or unavailable, falling back to client CSV generation.', err);
  ...
  return (
    <div style={{ 
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${theme.primary}99, ${theme.secondary}99), url(${IMAGES.dashboardBg}) center/cover`,
      ...
      <div className="row g-4 mb-5">
        ...
        <div className="col-md-3">
          <div className="rounded-3 p-4 text-center bg-white shadow-sm h-100 d-flex flex-column justify-content-center"
               style={{ 
                 boxShadow: theme.cardShadow,
                 border: '1px solid rgba(0,0,0,0.05)'
               }}>
            <CircularProgress value={data ? data.recyclingRate : 0} delay={300} />
          </div>
        </div>
        <div className="col-md-3">
          <StatCard
      ...
      >
        <div className="row align-items-center g-4">
          <div className="col-md-3">
            <label className="form-label fw-semibold text-muted small mb-2">Date Range</label>
          ...
          <div className="col-md-3">
            <label className="form-label fw-semibold text-muted small mb-2">Report Type</label>
            <select className="form-select border-1" style={{ borderColor: 'rgba(0,0,0,0.1)' }}>
          ...
          >
            <div className="d-flex align-items-center mb-4">
              <div 
                ...
                style={{ background: `linear-gradient(135deg, ${theme.primary}20, ${theme.primary}10)` }}
              >
                <span style={{ color: theme.primary, fontSize: '20px' }}>📈</span>
              </div>
              <h6 className="fw-semibold mb-0" style={{ color: theme.textDark }}>
        ...
        <div className="col-md-5">
          ...
          >
            <div className="d-flex align-items-center mb-4">
              <div 
                className="rounded-circle p-2 me-3"
                style={{ background: `linear-gradient(135deg, ${theme.secondary}20, ${theme.secondary}10)` }}
              >
                <span style={{ color: theme.secondary, fontSize: '20px' }}>🥧</span>
              </div>
              <h6 className="fw-semibold mb-0" style={{ color: theme.textDark }}>
                Waste Distribution
              </h6>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  ...
                  labelStyle={{ fill: theme.textDark, fontSize: 12, fontWeight: 500 }}
                >
                  {pieData.map((entry, i) => (
